...
public abstract class ProtoImplementationBase
{
    ...
    {
        if (StatusCodeResponse.TryExtract(exn, out var status))
        {
            return WriteErrorAsync(logger, response, exn, status, cancellationToken);
        }
        response.StatusCode = 500;
        logger.LogError(exn, "Proto operation ha failed [cancelled = {Cancelled}].", cancellationToken.IsCancellationRequested);
        return JsonSerializer.SerializeAsync(
            response.Body,
            new ErrorDescriptor("generic_error", exn.Message),
